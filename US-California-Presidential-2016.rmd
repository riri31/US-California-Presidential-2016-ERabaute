...
ggplot(data = subset(data_campaign,cand_nm %in% main_candidates & contb_receipt_amt>0),
...
data_campaign.bycategory<-transform(data_campaign.bycategory,
                                    contbr_occupation_new= 
                                      ifelse(contbr_occupation %in% top_category,
                                            as.character(contbr_occupation),as.character('OTHER')))
...
# TOP 10 contributors occupation for Clinton
TOP10_TRUMP <- head(subset(data_campaign.bycategory, cand_nm=='Trump, Donald J.'),
...
# Display occupation for finalists donators.
ggplot(subset(data_campaign.bycity,
...
data_campaign.byzip.latlong <- data_campaign.byzip %>% full_join(zip.code, by = c("contbr_zip" = "Zip.Code" ))

data_campaign.byzip.latlong.losangeles <-
  subset(data_campaign.byzip.latlong,County=='Los Angeles' & cand_nm %in% finalists)


qmplot(Longitude, Latitude, data = data_campaign.byzip.latlong.losangeles,size = n/400,  color=cand_nm,show.legend=F, aes(alpha=I(0.5)))+
  facet_wrap(~cand_nm)
...
data_campaign.byzip.latlong.sandiego <-
  subset(data_campaign.byzip.latlong,County=='San Diego' & cand_nm %in% finalists)


qmplot(Longitude, Latitude, data = data_campaign.byzip.latlong.sandiego,size = n/400,  color=cand_nm, show.legend=F, aes(alpha=I(0.5)))+
  facet_wrap(~cand_nm)
```
...
data_campaign.byzip.latlong.california <-
  subset(data_campaign.byzip.latlong,State.Abbreviation=='CA' & cand_nm %in% finalists)


qmplot(Longitude, Latitude, data = data_campaign.byzip.latlong.california,size = n/8000,  color=cand_nm, show.legend=F,aes(alpha=I(0.1)))+
  facet_wrap(~cand_nm)
```
